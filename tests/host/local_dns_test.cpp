...
namespace {
#if defined(__linux__)
void put16(std::string &v,std::uint16_t n){const auto high=static_cast<unsigned char>((n>>8)&0xffU);const auto low=static_cast<unsigned char>(n&0xffU);v.push_back(static_cast<char>(high));v.push_back(static_cast<char>(low));}
...
std::string query(std::uint16_t id,const std::string &name,std::uint16_t type=1){std::string q;put16(q,id);put16(q,0x0100);put16(q,1);q.append(6,'\0');std::size_t p=0;while(p<name.size()){auto e=name.find('.',p);if(e==std::string::npos)e=name.size();const auto label_size=e-p;if(label_size>63)return {};q.push_back(static_cast<char>(static_cast<unsigned char>(label_size)));q.append(name,p,label_size);p=e+1;}q.push_back('\0');put16(q,type);put16(q,1);return q;}
std::string ask(std::uint16_t port,const std::string&q,const char *address="127.0.0.1"){int fd=::socket(AF_INET,SOCK_DGRAM,0);timeval t{0,200000};::setsockopt(fd,SOL_SOCKET,SO_RCVTIMEO,&t,sizeof(t));sockaddr_in a{};a.sin_family=AF_INET;a.sin_port=htons(port);::inet_pton(AF_INET,address,&a.sin_addr);::sendto(fd,q.data(),q.size(),0,reinterpret_cast<sockaddr*>(&a),sizeof(a));std::array<char,512>b{};const auto received=::recv(fd,b.data(),b.size(),0);::close(fd);if(received<=0)return {};return std::string(b.data(),static_cast<std::size_t>(received));}
