...
int Cli::run(int argc,const char *const argv[]) {
  ...
  const bool valid=command=="run"||command=="start"||command=="stop"||command=="restart"||command=="status"||command=="info"||command=="access"||command=="logs"; if(!valid) { std::cerr<<"Unknown command: "<<command<<'\n'; usage(); return 2; }
  bool follow=false; std::optional<std::string> name;
  if(command=="logs") { for(int i=2;i<argc;++i) { const std::string value(argv[i]); if(value=="-f"||value=="--follow") follow=true; else if(!name) name=value; else { command_usage(command); return 2; } } }
  ...
  if(command=="logs") { const auto path=NativeDataDirectory::path()/"logs"/(target->id.value()+".log"); std::uintmax_t offset=0; do { std::ifstream input(path, std::ios::binary); if(input) { input.seekg(static_cast<std::streamoff>(offset)); std::cout<<input.rdbuf()<<std::flush; const auto size=std::filesystem::file_size(path); offset=size; } if(!follow) break; std::this_thread::sleep_for(std::chrono::milliseconds(150)); } while(true); return 0; }
