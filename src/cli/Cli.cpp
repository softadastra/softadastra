...
int Cli::run(int argc,const char *const argv[]) {
  ...
  if(command=="remote") { if((argc!=3|| (std::string(argv[2])!="disable"&&std::string(argv[2])!="status"))&&(argc!=5||std::string(argv[2])!="enable")) { command_usage(command); return 2; } const auto r=client_.request(argc==3?(std::string(argv[2])=="status"?"remote status":"remote disable"):"remote enable "+std::string(argv[3])+" "+argv[4]); if(!r||*r=="error") { std::cerr<<"failed to update remote reachability\n"; return 1; } if(std::string(argv[2])=="status") { std::cout<<*r<<'\n'; return 0; } std::cout<<(*r=="remote 1"?"remote reachability: enabled\n":"remote reachability: disabled\n"); return 0; }
  if(command=="register") { if(argc<4) { command_usage(command); return 2; } int executable=3; std::optional<AccessPoint> access; if(std::string(argv[3])=="--access") { if(argc<7||std::string(argv[5])!="--") { command_usage(command); return 2; } access=parse_access(argv[4]); if(!access) { std::cerr<<"access point must use http:port or https:port with port 1 to 65535\n"; return 2; } executable=6; } else if(std::string(argv[3])=="--") { if(argc<5) { command_usage(command); return 2; } executable=4; } const std::string software_name(argv[2]); const SoftwareId id(software_name); if(client_.software(id)) { std::cerr<<"Software already registered: "<<id.value()<<"\n\nChoose a different software name.\n"; return 1; } std::vector<std::string> args; for(int i=executable+1;i<argc;++i) args.emplace_back(argv[i]); std::error_code ec; const auto root=std::filesystem::weakly_canonical(std::filesystem::current_path(),ec); if(ec) return 1; if(!client_.register_software(id,ProcessSpec(argv[executable],std::move(args),root.string()),access,std::nullopt,software_name)) { std::cerr<<"failed to register software: "<<id.value()<<'\n'; return 1; } std::cout<<"registered: "<<id.value()<<'\n'; return 0; }
