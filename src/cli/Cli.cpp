...
std::string command_name(const SoftwareEntry &e) { const auto &s=e.process_spec(); if(s.executable()=="/bin/sh" && s.arguments().size()==2 && s.arguments()[0]=="-lc") return s.arguments()[1]; std::string v=s.executable(); for(const auto &a:s.arguments()) v+=" "+a; return v; }
...
bool print_access(ControlClient &client,const Target &t) {
  ...
  const auto host=client.local_access(); if(!host||host->primary_ipv4.empty()) { std::cerr<<"current IPv4 address is unavailable\n"; return false; } const auto url=configured->protocol()==AccessProtocol::Http?AccessUrl::http(host->primary_ipv4,configured->port()):AccessUrl::https(host->primary_ipv4,configured->port()); std::cout<<"Software: "<<t.name<<"\nHost:     "<<(host->host_name.empty()?"unavailable":host->host_name)<<"\nURL:      "<<url<<"\n\n"; if(!QrCode::print(url)) std::cout<<"QR generation is unavailable.\n"; std::cout<<"\nScan with your phone.\n"; return true;
}
}
...
int Cli::run(int argc,const char *const argv[]) {
  ...
  if(command=="help") { if(argc==2) usage(); else if(argc==3) command_usage(argv[2]); else { std::cerr<<"help accepts one command\n"; return 2; } return 0; }
  if(is_help(command)) { usage(); return 0; }
  if(argc>=3&&is_help(argv[2])) { if(argc!=3) { command_usage(command); return 2; } command_usage(command); return 0; }
  if(command=="init") {
  ...
  const bool valid=command=="run"||command=="start"||command=="stop"||command=="restart"||command=="status"||command=="info"||command=="access"; if(!valid) { std::cerr<<"Unknown command: "<<command<<'\n'; usage(); return 2; } if(argc>3) { command_usage(command); return 2; } const auto name=argc==3?std::optional<std::string>(argv[2]):std::nullopt;
