...
int Cli::run(int argc,const char *const argv[]) {
  ...
  if(command=="network") { if(argc!=3||std::string(argv[2])!="info") { command_usage(command); return 2; } const auto capability=network_?std::optional<NetworkCapability>(network_->network_capability()):client_.network_capability(); if(!capability) { std::cerr<<"Host network information is unavailable\n"; return 1; } std::cout<<"State:            "<<network_state_name(capability->state)<<"\nPrimary IPv4:     "<<(capability->primary_ipv4.empty()?"-":capability->primary_ipv4)<<"\nInterface:        "<<(capability->primary_interface.empty()?"-":capability->primary_interface)<<"\nType:             "<<network_interface_type_name(capability->interface_type)<<"\nLocal network:    "<<local_network_state_name(capability->local_network_state)<<"\nManaged network:  "<<managed_network_capability_name(capability->managed_network_capability)<<'\n'; return 0; }
  if(command=="init") {
