    ...
    char **argv)
{
  ...
  {
    std::cerr
        << "usage: softadastra-gateway --listen <ipv4>:<port> "
  ...
  softadastra::ControlLocalGatewayTargetResolver resolver(
      client);

  softadastra::LocalGateway gateway(
      resolver);
  ...
  std::cerr
      << "softadastra-gateway is supported on Linux only\n";
