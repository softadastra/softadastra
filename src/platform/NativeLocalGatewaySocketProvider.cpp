...
#include <unistd.h>
#endif
namespace softadastra { NativeLocalGatewaySocketProvider::NativeLocalGatewaySocketProvider(Environment environment)noexcept:environment_(std::move(environment)){if(!environment_)environment_=[](const char *key)->const char*{static thread_local std::string value;const auto environment=environment_value(key);if(!environment)return nullptr;value=*environment;return value.c_str();};} LocalGatewaySocket NativeLocalGatewaySocketProvider::acquire()const noexcept{
#if defined(__linux__)
