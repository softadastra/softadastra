...
namespace softadastra
{
  namespace
  {
    ...
    constexpr const char *header_v8 = "softadastra-registrations 8\n";
        ...
        std::size_t &offset)
    {
      ...
      {
        const auto value =
            std::stoull(text.substr(offset, end - offset));
      ...
      const std::vector<SoftwareEntry> &entries)
  {
    ...
    for (const auto &entry : entries)
    {
      ...
      output +=
          entry.process_spec().working_directory().has_value()
              ? "1\n"
              : "0\n";
      ...
      if (entry.process_spec().working_directory().has_value())
      {
        append_string(
            output,
            entry.process_spec().working_directory().value());
      }
      ...
      output +=
          std::to_string(
              entry.process_spec().arguments().size());
      ...
      output += entry.desired_running() ? "1\n" : "0\n";
    }
  ...
  SoftwareRegistrationFormat::deserialize(
      const std::string &text)
  {
    const bool version_eight =
        text.starts_with(header_v8);

    const bool version_seven =
        text.starts_with(header_v7);

    const bool version_six =
        text.starts_with(header_v6);

    const bool version_five =
        text.starts_with(header_v5);

    const bool version_four =
        text.starts_with(header_v4);

    const bool version_three =
        text.starts_with(header_v3);

    const bool version_two =
        text.starts_with(header_v2);
         ...
         ++index)
    {
      ...
      if (version_six || version_seven || version_eight)
      {
        const auto value =
            read_string(text, offset);
      ...
      if (version_eight)
      {
        const auto value = read_count(text, offset);
      ...
      entries.back().set_desired_running(desired_running);

      for (std::size_t previous = 0;
           previous + 1 < entries.size();
           ++previous)
      {
        if (entries[previous].id() == entries.back().id() ||
            (!entries.back().name().empty() &&
             entries[previous].name() == entries.back().name()))
        {
          return std::nullopt;
        }
      }
