...
namespace softadastra
{
  namespace
  {
    ...
    std::optional<std::string> certificate_id(X509 *certificate)
    {
      ...
      const bool extracted =
          key != nullptr &&
          EVP_PKEY_get_raw_public_key(key, public_key.data(), &public_key_size) == 1 &&
          EVP_Q_digest(
              nullptr,
              "SHA256",
              nullptr,
      ...
      return hexadecimal(digest.data(), digest_size);
    }
  } // namespace

  HostPeerClient::HostPeerClient(HostPeerTrust trust, std::uint16_t port) noexcept
      ...
      std::string_view command) const noexcept
  {
    ...
    if (command != "identity" && command != "ping" && command != "infrastructure")
    {
      return std::nullopt;
    }
    ...
    if (descriptor < 0 ||
        ::inet_pton(AF_INET, trust_.address().c_str(), &address.sin_addr) != 1 ||
    ...
    if (SSL_connect(ssl) == 1)
    {
      ...
      const auto presented_id = certificate != nullptr
                                    ? certificate_id(certificate)
                                    : std::optional<std::string>{};
      X509_free(certificate);

      if (presented_id && trust_.accepts(*presented_id))
      {
            ...
            static_cast<int>(wire_request.size()))
        {
          ...
          const int size = SSL_read(ssl, response.data(), static_cast<int>(response.size()));

          if (size > 0)
          {
            result = std::string(response.data(), static_cast<std::size_t>(size));

            if (command == "identity" && *result != "identity " + *presented_id + "\n")
            {
              result.reset();
            }
          }
