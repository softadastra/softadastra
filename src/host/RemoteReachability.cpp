...
bool receive_all(softadastra::NativeSocket fd, char *out, std::size_t size)
{
...
bool read_frame(softadastra::NativeSocket fd,std::string &frame){std::array<char,4> h{};if(!receive_all(fd,h.data(),h.size()))return false;auto n=u32(h.data());if(n==0||n>max_frame)return false;frame.resize(n);return receive_all(fd,frame.data(),n);}
bool write_frame(softadastra::NativeSocket fd,const std::string &frame){if(frame.size()>max_frame)return false;std::string h;put32(h,static_cast<std::uint32_t>(frame.size()));return send_all(fd,h)&&send_all(fd,frame);}
...
softadastra::NativeSocket connect_to(const softadastra::RemoteEndpoint &endpoint,std::atomic<softadastra::NativeSocket> &active){
#if defined(_WIN32)
 ...
 if(!winsock_ready)return softadastra::InvalidSocket;
#endif
addrinfo hints{};hints.ai_family=AF_UNSPEC;hints.ai_socktype=SOCK_STREAM;addrinfo *found{};const auto port=std::to_string(endpoint.port);if(::getaddrinfo(endpoint.address.c_str(),port.c_str(),&hints,&found)!=0)return softadastra::InvalidSocket;softadastra::NativeSocket fd=softadastra::InvalidSocket;for(auto *it=found;it;it=it->ai_next){fd=::socket(it->ai_family,it->ai_socktype,it->ai_protocol);if(fd==softadastra::InvalidSocket)continue;active=fd;if(::connect(fd,it->ai_addr,it->ai_addrlen)==0)break;if(active.exchange(softadastra::InvalidSocket)==fd)close_socket(fd);fd=softadastra::InvalidSocket;}::freeaddrinfo(found);return fd;}
}
...
void RemoteReachability::disable()noexcept{stopping_=true;const auto fd=socket_.exchange(InvalidSocket);if(fd!=InvalidSocket){
#if defined(_WIN32)
::shutdown(fd,SD_BOTH);
#else
...
void RemoteReachability::run()noexcept{HttpProxy proxy(resolver_);auto delay=std::chrono::milliseconds(50);while(!stopping_){RemoteEndpoint endpoint;{std::lock_guard lock(mutex_);endpoint=endpoint_;state_=RemoteReachabilityState::Connecting;}NativeSocket fd=connect_to(endpoint,socket_);if(fd==InvalidSocket||stopping_){if(fd!=InvalidSocket&&socket_.exchange(InvalidSocket)==fd)close_socket(fd);break;}delay=std::chrono::milliseconds(50);{std::lock_guard lock(mutex_);if(!stopping_)state_=RemoteReachabilityState::Ready;}bool ok=true;while(!stopping_&&ok){std::string frame,software;HttpProxyRequest request;ok=read_frame(fd,frame)&&parse_request(frame,software,request);if(ok){std::string answer(1,2);answer+=proxy.forward(software,request);ok=write_frame(fd,answer);}}if(socket_.exchange(InvalidSocket)==fd)close_socket(fd);if(!stopping_){std::unique_lock lock(mutex_);state_=RemoteReachabilityState::Degraded;wake_.wait_for(lock,delay,[this]{return stopping_.load();});delay=std::min(delay*2,std::chrono::milliseconds(1000));}}}
}
